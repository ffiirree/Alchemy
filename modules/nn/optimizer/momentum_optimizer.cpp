...
template<typename T>
MomentumOptimizer<T>::MomentumOptimizer(const OptimizerParameter &param)
        : Optimizer<T>(param)
{
...
template<typename T>
void MomentumOptimizer<T>::optimize()
{
...
template<typename T>
void MomentumOptimizer<T>::update()
{
    const auto& learnable_params = this->net_->learnable_params();
    ...
    if(Global::mode() == Global::CPU) {
        for(size_t idx = 0; idx < learnable_params.size(); ++idx) {
            ...
            vector_axpy(buf2_[idx].count(), (T)1.0, buf_[idx].cptr(), std::get<0>(learnable_params[idx])->mutable_data_cptr());
        }
    }
    else {
        for(size_t idx = 0; idx < learnable_params.size(); ++idx) {
